...
scores <- read.csv("https://www.masseyratings.com/scores.php?s=308075&sub=11590&all=1&mode=2&format=1", header=FALSE)
teams <- read_csv("https://www.masseyratings.com/scores.php?s=308075&sub=11590&all=1&mode=2&format=2", col_names = FALSE)
...
library(readr)
write_csv(rankedteams, paste("2019 Rankings/D1 MOV RW ", format(Sys.time(),"%Y %m %d"),".csv",sep=""))

FBSteams <- read_csv("https://www.masseyratings.com/scores.php?s=308075&sub=11604&all=1&mode=2&format=2", col_names=FALSE)
FBSRanking<-filter(rankedteams, Team %in% FBSteams$X2)

write.csv(FBSRanking, paste("2019 Rankings/D1-FCS MOV RW ", format(Sys.time(),"%Y %m %d"),".csv",sep=""), row.names = FALSE)
